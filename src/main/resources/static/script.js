...
//==============КНОПОЧКИ==============
    $("#mainTableWithUsers").find('button').on('click', (event) => {
        let dfltModal = $('#defaultModal');

        let targetButton = $(event.target);
        let buttonUserId = targetButton.attr('data-userid');
        let buttonAction = targetButton.attr('data-action');

        dfltModal.attr('data-userid', buttonUserId);
        dfltModal.attr('data-action', buttonAction);
        dfltModal.modal('show')
    })
}

// ===========получение списка ролей с базы
const roleJson = []
...
async function deleteUser(modal, id) {
    let currentUser = await userFetchService.showOneUser(id)
    let user = currentUser.json()
    let modalForm = $(`#defaultModal`)
        ...
        let bodyForm = `<form id="deleteUser">
                            <div class="col-md-7 offset-md-3 text-center">
                                <div class="form-group">
                                    ...
                                    <input type="number" value="${user.id}" name="id" class="form-control" readonly>
                                </div>
                                <div class="form-group">
                                    <span class="font-weight-bold">First name</span>
                                    <input type="text" value="${user.firstName}" name="name" class="form-control" readonly>
                                </div>
                                <div class="form-group">
                                    <span class="font-weight-bold">Last name</span>
                                    <input type="text" value="${user.lastName}" name="surname" class="form-control" readonly>
                                </div>
                                <div class="form-group">
    ...
    user.then(user => {
        let bodyForm = `<form id="editUser">
                            <div class="col-md-7 offset-md-3 text-center">
                                <div class="form-group">
                                    <span class="font-weight-bold">ID</span>
                                    <input type="number" value="${user.id}" name="id" id="id" class="form-control" readonly>
                                </div>
                                <div class="form-group">
                                    <span class="font-weight-bold">First name</span>
                                ...
                                <div class="form-group">
                                    <span class="font-weight-bold">Last name</span>
                                    <input type="text" value="${user.lastName}" name="surname" id="surname" class="form-control">
                                </div>
                                <div class="form-group">
                                    <span class="font-weight-bold">Age</span>
